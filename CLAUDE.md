...
计划文件写好后，必须为每个涉及的子项目生成一段交接话术，直接粘贴给对应子项目的 CC 实例启动开发。

- 只涉及前端：写一段给 `web/` CC
- 只涉及后端：写一段给 `api/` CC
- 前后端都涉及：分别写两段，各自独立，不要合并

交接话术要包含：计划文件路径（`plan/PLN-XXX-slug.md`）、任务目标一句话概括、需要特别注意的约束或依赖。格式示例：

```
请先读 plan/PLN-XXX-slug.md，然后按计划实现 [任务目标]。注意：[关键约束或跨边界依赖]。
```
