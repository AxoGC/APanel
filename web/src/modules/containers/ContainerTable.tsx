          ...
          return (
            <div key={container.id} className="flex flex-col gap-2 px-2 py-3 hover:bg-gray-100 dark:hover:bg-gray-800 md:flex-row md:items-center md:gap-3 md:py-2">
              ...
              <div className="flex flex-col gap-2 md:hidden">
                <div className="flex items-center justify-between gap-2"><span className="truncate text-sm text-gray-900 dark:text-gray-100">{container.name}</span><div className="flex shrink-0 items-center gap-1.5"><span className={cn('size-1.5 rounded-full', dot)} /><span className={cn('text-xs', text)}>{state}</span></div></div>
                <div className="truncate text-xs text-gray-500">{formatContainerStatus(container.status, t)}</div>
